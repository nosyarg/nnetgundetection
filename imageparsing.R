  ...
  setwd("/Users/grayson/Desktop/Conrad Files/NerfGunConradTrainingPhotos")#input directory of training images here
  ...
  bw.image <- channel(color.image,"red") - channel(color.image,"green")#convert the image from RGB to black and white
  ...
  writeImage(bw.image,paste("tempimages/",1,".jpg",sep='',collapse=''))
  ...
  write.table(matrix(data,datasize),file="nerfgundata.csv",row.names = FALSE, col.names = FALSE,sep=",")#write compiled dataset to a csv file
  #for(i in 1:length(names))#rename all images to 1,2,3,4... to facilitate negative mining
  #{
  #  imagetmp <- readImage(names[i])#read the current image to our memory
  #  writeImage(imagetmp,paste("images/",i,".jpg",sep='',collapse=''))#write the renamed images to the new files
